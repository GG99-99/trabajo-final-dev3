...
async function enqueueWrite(request, bodyText) {
  ...
  console.log(`[SW-Checkout] encolado: ${request.method} ${request.url}`)
...
async function drainQueue() {
  ...
  console.log(`[SW-Checkout] drenando cola: ${items.length} items`)
  ...
  for (const item of items) {
    try {
      ...
      if (res.ok || res.status < 500) {
        ...
        console.log(`[SW-Checkout] sincronizado: ${item.method} ${item.url} → ${res.status}`)
      } else {
      ...
      if (item.retries >= 10) {
        console.warn(`[SW-Checkout] descartando item por exceder reintentos: ${item.url}`)
        await idbDelete(STORE_QUEUE, item.id)
      } else {
...
async function runHealthCheck() {
  ...
  if (alive !== _coreAlive) {
    _coreAlive = alive
    console.log(`[SW-Checkout] core ${alive ? 'recuperado ✓' : 'caído ✗'}`)
    notifyClients({
      type:         'CORE_STATUS',
      alive,
...
self.addEventListener('fetch', (event) => {
  const { request } = event
  if (!request.url.startsWith(CORE_ORIGIN)) return
...
async function handleGet(request) {
  try {
    ...
    if (cached) {
      console.warn(`[SW-Checkout] sirviendo desde cache: ${request.url}`)
