...
export const punchService = {
    ...
    clockIn: async (worker_id) => {
        // Validate worker exists
        const worker = await prisma.worker.findUnique({ where: { worker_id } });
        if (!worker) {
            throw { statusCode: 404, name: 'WorkerNotFound', message: 'Trabajador no encontrado.' };
        }
        ...
        if (existing) {
            throw { statusCode: 409, name: 'AlreadyClockedIn', message: 'Ya registraste tu entrada hoy.' };
        }
    ...
    clockOut: async (worker_id) => {
        ...
        if (!assist) {
            throw { statusCode: 409, name: 'NotClockedIn', message: 'No has registrado tu entrada hoy.' };
        }
        if (assist.close) {
            throw { statusCode: 409, name: 'AlreadyClockedOut', message: 'Ya registraste tu salida hoy.' };
        }
    ...
    getTodayStatus: async (worker_id) => {
        const attendance = await prisma.attendance.findFirst({
            where: { day: todayStr(), is_deleted: false }
        });
        if (!attendance)
            return { clocked_in: false, clocked_out: false, assist: null };
        const assist = await prisma.assist.findUnique({
            where: { worker_id_attendance_id: { worker_id, attendance_id: attendance.attendance_id } },
            include: { worker: { include: { person: true } }, attendance: true }
        });
