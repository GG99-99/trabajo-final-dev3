...
export const imgController = {
   ...
   */
  getRaw: async (req: Request, res: Response) => {
    ...
    if (!img.source) return res.status(404).send('No image data')
  ...
  create: async (req: Request, res: Response) => {
    ...
    if (!file) return res.status(400).json({ ok: false, data: null, error: { name: 'BadRequest', statusCode: 400, message: 'No file uploaded' } })
