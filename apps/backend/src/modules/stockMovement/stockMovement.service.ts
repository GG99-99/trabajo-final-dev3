/**
 * stockMovement.service.ts — con resiliencia Redis
 * Circuit breaker en lecturas y operaciones dentro de transacciones.
 * Las escrituras ocurren siempre dentro de tx (bill, inventory),
 * por lo que el circuit breaker se aplica en el nivel superior (bill.service).
 */
...
export const stockMovementService = {
  get: async (filters: GetStockMovementFilters, tx?: Prisma.TransactionClient) => {
    if (tx) return await stockMovementModel.get(filters, tx)
    return await withCircuitBreaker(() => stockMovementModel.get(filters))
  },

  getCost: async (stock_movement_id: number, tx?: Prisma.TransactionClient) => {
    const st = tx
      ? await stockMovementModel.get({ stock_movement_id }, tx)
      : await withCircuitBreaker(() => stockMovementModel.get({ stock_movement_id }))
  ...
  create: async (data: CreateStockMovement, tx: Prisma.TransactionClient) => {
    if (data.type !== 'entry') {
      const item = await inventoryModel.get(
        { inventory_item_id: data.inventory_item_id, gte: data.quantity },
        tx
      )
      if (!item)
        throw {
          ...
          statusCode: 400,
          message: 'no hay cantidad suficiente para realizar extraccion del inventario',
        } as ApiErr
    }
    ...
    await inventoryService.updateQuantity(
      { inventory_item_id: data.inventory_item_id, quantity: data.quantity, type: data.type },
      tx
    )
  ...
  createForProductVariant: async (data: CreateStockMovementFromProduct, tx: Prisma.TransactionClient) => {
    const item = await inventoryService.getNotExpired({ product_variant_id: data.product_variant_id }, tx)
    if (!item)
      throw {
        name: 'InsufficientQuantity',
        statusCode: 400,
    ...
    if (Number(item.current_quantity) < data.quantity)
      throw {
        name: 'InsufficientQuantity',
        statusCode: 400,
    ...
    return await stockMovementService.create(
      { inventory_item_id: item.inventory_item_id, reason: data.reason, quantity: data.quantity, type: data.type },
      tx
    )
  },
